...
export function ServiceCard({
  ...
  return (
    <Link
      ...
      className="group flex flex-col overflow-hidden rounded-3xl border border-navy-100/70 bg-white transition-all duration-300 hover:-translate-y-1 hover:border-orange-200 hover:shadow-[0_24px_60px_-20px_rgba(1,47,135,0.25)]"
    >
      <div className={`relative h-36 overflow-hidden bg-gradient-to-br ${gradientForServiceIcon(service.icon)}`}>
        <EditorialMedia
      ...
      <div className="flex flex-1 flex-col p-6">
        <h3 className="font-display text-xl font-bold text-navy-800">{t(service.title, locale)}</h3>
        <p className="mt-2 flex-1 text-sm leading-relaxed text-navy-500">{t(service.tagline, locale)}</p>
...
export function ProjectCard({
  ...
  return (
    <Link
      ...
      className="group flex flex-col overflow-hidden rounded-3xl border border-navy-100/70 bg-white transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_24px_60px_-20px_rgba(1,47,135,0.28)]"
    >
      <div className={`relative aspect-[16/10] overflow-hidden bg-gradient-to-br ${project.color}`}>
        ...
        <div className="absolute inset-0 flex flex-col justify-between p-6 text-white">
          ...
          <p className="font-display text-2xl font-extrabold leading-tight drop-shadow-md">{project.client}</p>
        </div>
      </div>
      <div className="flex flex-1 flex-col p-6">
        <h3 className="font-display text-lg font-bold text-navy-800">{t(project.title, locale)}</h3>
        <p className="mt-2 flex-1 text-sm leading-relaxed text-navy-500">{t(project.cover, locale)}</p>
...
export function PostCard({
  ...
  return (
    <Link
      ...
      className="group flex flex-col overflow-hidden rounded-3xl border border-navy-100/70 bg-white transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_24px_60px_-20px_rgba(1,47,135,0.22)]"
    >
      <div className={`relative aspect-[16/9] overflow-hidden bg-gradient-to-br ${post.color}`}>
        ...
        <span className="absolute left-5 top-5 z-10 rounded-full bg-white/90 px-3 py-1 text-xs font-bold uppercase tracking-wider text-navy-700">
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <h3 className="mt-3 font-display text-lg font-bold leading-snug text-navy-800 group-hover:text-orange-600">
          {t(post.title, locale)}
        </h3>
        <p className="mt-2 flex-1 text-sm leading-relaxed text-navy-500">{t(post.excerpt, locale)}</p>
        <span className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-navy-700 group-hover:text-orange-600">
          {readLabel}
          <Icon name="arrow" className="h-4 w-4 transition-transform group-hover:translate-x-1" />
        </span>
      </div>
...
export function FeaturedPost({
  ...
  return (
    <Link href={localePath(locale, `/blog/${post.slug}`)} className="group grid overflow-hidden rounded-[2rem] bg-navy-950 text-white shadow-[0_24px_70px_-28px_rgba(1,23,70,0.6)] md:grid-cols-2">
      <div className="relative min-h-64 overflow-hidden bg-navy-900">
      ...
      <div className="flex flex-col justify-center p-7 sm:p-10">
        ...
        <h2 className="mt-4 font-display text-2xl font-extrabold leading-tight sm:text-3xl">{t(post.title, locale)}</h2>
        <p className="mt-4 text-sm leading-relaxed text-white/80 sm:text-base">{t(post.excerpt, locale)}</p>
        <span className="mt-6 inline-flex items-center gap-2 font-semibold text-white group-hover:text-orange-300">{readLabel}<Icon name="arrow" className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span>
      </div>
    </Link>
  );
}
