...
function BlockRenderer({ block, locale, headingId }: { block: Block; locale: Locale; headingId?: string }) {
  switch (block.type) {
    ...
    case "callout": {
      ...
      const styles = {
        tip: {
          ...
          icon: "💡",
          badge: locale === "vi" ? "Mẹo chiến lược" : "Pro Tip",
        },
        warning: {
          bg: "bg-amber-50/80 border-amber-500 text-amber-950",
          icon: "⚠️",
          badge: locale === "vi" ? "Lưu ý quan trọng" : "Important Note",
        },
        info: {
          bg: "bg-blue-50/80 border-blue-500 text-blue-950",
          icon: "ℹ️",
          badge: locale === "vi" ? "Thông tin hữu ích" : "Key Insight",
        },
      }[variant];

      return (
        <aside className={`my-6 sm:my-8 rounded-2xl border-l-4 p-4 sm:p-6 shadow-xs ${styles.bg}`}>
    ...
    case "table":
      return (
        <div className="my-6 sm:my-8 overflow-hidden rounded-2xl sm:rounded-3xl border border-navy-100/80 bg-white shadow-xs">
